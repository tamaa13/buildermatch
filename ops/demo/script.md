...
| 0:35 – 0:45 | Cut to BscScan: the VerdictRegistry contract page, then the minted NFT (tokenId + tx hash), then click IPFS link → reasoning JSON opens | *"The whole debate is pinned to IPFS. A verdict NFT mints on BNB. Forever on-chain, forever auditable."* (18 w) | Use a cursor highlight tool; slow down the click moments |
...
- [ ] `demo/raw/bscscan-nft.mov` — capture of BscScan contract page → NFT tokenId → IPFS
...
>
> The whole debate is pinned to IPFS. A verdict NFT mints on BNB. Forever on-chain, forever auditable.
>
