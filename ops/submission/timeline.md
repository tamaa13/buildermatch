...
| --- | --- | --- | --- |
| T–04:00 (17:00) | contract-dev | Chapel testnet deploy confirmed, CONTRACT_ADDRESS pasted to all .env files | faucet claim |
| T–04:00 (17:00) | agent-backend | Real-E2E dry run against Chapel, produce `fixtures/real-run-A.json` + `real-run-B.json` | ANTHROPIC_API_KEY + token addresses |
...
| T–01:00 (20:00) | user | Fill out DoraHacks form from `ops/submission/dorahacks.md` → save as draft | all assets ready |
...
- [ ] Chapel contract verified on BscScan (nice-to-have, not blocker)
...
- If **Chapel deploy** is not ready by 20:30: submit with anvil local screenshots + a note "Chapel deploy in flight, see README for local run instructions"
